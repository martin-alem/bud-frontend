...
function ExpenseCard(props) {
  ...
  const cardColor = stringToColor(expense.category);
  return (
        ...
        {!editMode ? (
          <>
            <Typography variant="h2" component="h2" sx={{ fontWeight: "bolder" }}>
              $ {expense.amount.toString().includes(".") ? expense.amount.toString() : expense.amount.toString().padEnd(expense.amount.toString().length + 3, ".00")}
            </Typography>
          </>
        ) : (
          <>
